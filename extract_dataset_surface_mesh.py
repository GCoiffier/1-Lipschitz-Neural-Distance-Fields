...
def extract_train_point_cloud(n_surf, n_train, mesh, domain):
    ...
    domain.pad(0.95)
    X_other2 = M.sampling.sample_AABB(domain, 5*n_train)
...
def extract_train_point_cloud_unsigned(n_pt, mesh, domain):
    ...
    domain.pad(0.95)
    X_out2 = M.sampling.sample_AABB(domain, n_pt//2)
...
def extract_train_point_cloud_distances(n_surf, n_pt, mesh, domain):
    ...
    domain.pad(1.95)
    X_out2 = M.sampling.sample_AABB(domain, n_pt//10)
    X_out = np.concatenate((X_out1, X_out2))
...
if __name__ == "__main__":
    ...
    match args.mode:
        ...
        case "unsigned":
            if args.importance_sampling:
                X_out1 = M.sampling.sample_AABB(domain, 10*args.n_train)
    ...
    test_bb.pad(0.9)
    X_other_test2 = M.sampling.sample_AABB(test_bb, args.n_test//2)
